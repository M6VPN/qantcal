...
)
{
	...
	reference.name = name;
	reference.service = BandService::Amateur;
	...
	reference.mode_notes = QStringLiteral("Amateur operating modes depend on licence, band segment, and local band plan.");
...
QVector<BandReference>
band_references(BandReferenceFilter filter)
{
	QVector<BandReference> references;

	for (const BandReference &band : band_references()) {
		if (filter == BandReferenceFilter::All) {
			references.append(band);
		} else if (filter == BandReferenceFilter::Amateur && band.service == BandService::Amateur) {
			references.append(band);
		} else if (filter == BandReferenceFilter::BroadcastReference && band.service != BandService::Amateur) {
			references.append(band);
		}
	}
...
QString
band_service_key(BandService service)
{
	switch (service) {
	case BandService::Amateur:
		return QStringLiteral("amateur");
...
QString
band_service_label(BandService service)
{
	switch (service) {
	case BandService::Amateur:
		return QStringLiteral("Amateur");
...
BandService
band_service_from_key(const QString &key)
{
	if (key.compare(QStringLiteral("amateur"), Qt::CaseInsensitive) == 0)
		return BandService::Amateur;
	if (key.compare(QStringLiteral("broadcast"), Qt::CaseInsensitive) == 0)
